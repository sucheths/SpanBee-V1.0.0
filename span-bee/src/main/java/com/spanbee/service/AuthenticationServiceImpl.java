          ...
          if (customer != null) {
            ...
            if (status == Constants.STATUS_ACTIVE) {
              ...
              if (password.equals(decryptedPass)) {
                sessionId =
                    new KeyGenerator().generateSessionID(AESSecurity.decrypt(customer
                        .getEmailAddress()));
                ...
                if (sessionFlag) {
                  message =
                      PropertyReader.resourceBundlesManager.getValueFromResourceBundle(
                          Constants.LANGUAGE, Constants.AUTHENTICATION_SUCCESS);
                  response.setCode(Constants.HTTP_STATUS_CODE_SUCCESS);
                  response.setMessage(message);
                  response.setSessionId(sessionId);
                  response.setNextPage("welcome_page");
                  response.setStatus(Constants.RESPONSE_SUCCESS);
                  LOGGER.fatal("Login success for customer with unique id ::"
                  ...
                  LOGGER.fatal("Failed to update session id for customer with unique id ::"
                      + customer.getUniqueId());
                }
                ...
                message =
                    PropertyReader.resourceBundlesManager.getValueFromResourceBundle(
                        Constants.LANGUAGE, Constants.ERROR_CODE_501 + Constants._ERROR_MESSAGE);
                response.setCode(Constants.ERROR_CODE_501);
                response.setMessage(message);
                response.setStatus(Constants.RESPONSE_FAILURE);
                LOGGER
                    .fatal("Login failed for customer with unique id ::" + customer.getUniqueId());
              }
            } else if (status == Constants.STATUS_INACTIVE) {
              message =
                  PropertyReader.resourceBundlesManager.getValueFromResourceBundle(
                      Constants.LANGUAGE, Constants.ERROR_CODE_510 + Constants._ERROR_MESSAGE);
              response.setCode(Constants.ERROR_CODE_510);
              response.setMessage(message);
              response.setStatus(Constants.RESPONSE_FAILURE);
              LOGGER.fatal("Status is inactive for the customer with unique id ::"
                  + customer.getUniqueId());
            }
      ...
      if (customer != null) {
        ...
        if (status == Constants.STATUS_ACTIVE) {
          ...
          if (Constants.GF_PASSWORD.equals(decryptedPass)) {
            sessionId =
                new KeyGenerator()
                    .generateSessionID(AESSecurity.decrypt(customer.getEmailAddress()));
            sessionFlag = authenticationDao.setCustomerSessionId(sessionId, customer.getId());
            if (sessionFlag) {
              message =
                  PropertyReader.resourceBundlesManager.getValueFromResourceBundle(
